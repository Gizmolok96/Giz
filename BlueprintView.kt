...
import com.potolochnik.app.data.CeilingElement
import com.potolochnik.app.data.CeilingElementType
...
class BlueprintView @JvmOverloads constructor(
    ...
    private fun drawLines(canvas: Canvas) {
        ...
        lines.forEachIndexed { index, line ->
            ...
            if (showDimensions) {
                ...
                val nx = -dy / pixelLen * (20f / scaleFactor)
                val ny =  dx / pixelLen * (20f / scaleFactor)

                if (line.measuredLength != null) {
                    // Confirmed measurement — draw in blue
                    canvas.drawText("${line.measuredLength!!.toInt()}", midX + nx, midY + ny, paintDimText)
                } else if (!isClosed) {
                    // Live pixel distance while drawing — draw in dimmer color as rough guide
                    val liveText = pixelLen.toInt().toString()
                    canvas.drawText(liveText, midX + nx, midY + ny, paintLiveDimText)
                }
            }
        }
    }
    ...
    private fun drawDiagonals(canvas: Canvas) {
        ...
        diagonals.forEach { diag ->
            val p1 = pointMap[diag.fromId] ?: return@forEach
            val p2 = pointMap[diag.toId] ?: return@forEach

            val isActive = diag.id == activeLineId
            val paint = if (isActive) paintLineActive else paintDiagonal
            canvas.drawLine(p1.x, p1.y, p2.x, p2.y, paint)

            if (showDimensions && diag.measuredLength != null) {
                val midX = (p1.x + p2.x) / 2
                val midY = (p1.y + p2.y) / 2
                canvas.drawText("${diag.measuredLength!!.toInt()}", midX, midY - (12f / scaleFactor), paintDiagText)
            }
        }
    }
    ...
    private fun drawAngles(canvas: Canvas) {
        val n = points.size
        val textOffset = 38f / scaleFactor
        paintAngleText.textSize = 20f / scaleFactor

        // Определяем ориентацию обхода через формулу Shoelace.
        // В экранных координатах (Y растёт вниз):
        //   shoelace > 0 → обход по часовой стрелке (CW)
        //   shoelace < 0 → обход против часовой стрелки (CCW)
        var shoelace = 0.0
        for (i in 0 until n) {
            val j = (i + 1) % n
        ...
        for (i in 0 until n) {
            ...
            // Векторы из curr к предыдущей и следующей точке
            ...
            // Знаковое векторное произведение: положительное = поворот против CW
            ...
            // Определяем, является ли угол рефлексивным (> 180°).
            // Для CW-обхода (shoelace > 0): рефлексивный, если cross > 0 (CCW-поворот).
            // Для CCW-обхода (shoelace < 0): рефлексивный, если cross < 0 (CW-поворот).
            val isReflex = if (shoelace > 0) crossSigned > 0 else crossSigned < 0
            if (isReflex) angleDeg = 360 - angleDeg

            val aLen = sqrt(ax * ax + ay * ay).coerceAtLeast(0.001f)
            val bLen = sqrt(bx * bx + by * by).coerceAtLeast(0.001f)
            val dirX = (ax / aLen + bx / bLen)
            val dirY = (ay / aLen + by / bLen)
            val dirLen = sqrt(dirX * dirX + dirY * dirY).coerceAtLeast(0.001f)
            val labelX = curr.x + (dirX / dirLen) * textOffset
            val labelY = curr.y + (dirY / dirLen) * textOffset
