...
import com.zamerpro.app.databinding.ActivityDrawingBinding
...
class DrawingActivity : AppCompatActivity() {
    ...
    // Исходные координаты скетча — сохраняются один раз при старте ввода размеров
    // и используются как стабильная опора для живой перестройки чертежа
    private var sketchOriginX: FloatArray? = null
    private var sketchOriginY: FloatArray? = null
    ...
    private fun setupBlueprint() {
        ...
        binding.blueprintView.onLineSelected = { line ->
            ...
            val label = if (p1 != null && p2 != null) {
                val lA = if (p1.label <= p2.label) p1.label else p2.label
                val lB = if (p1.label <= p2.label) p2.label else p1.label
                "$lA–$lB"
            } else ""
        ...
        // Режим выбора углов для диагонали на чертеже
        binding.blueprintView.onDiagonalFirstPicked = { p ->
            binding.tvElementModeHint.text = "Угол ${p.label} выбран — нажмите второй угол"
        }

        binding.blueprintView.onDiagonalPicked = { p1, p2 ->
            exitDiagonalPickMode()
            showDiagonalLengthInputWithNumpad(p1, p2)
        }
    }
    ...
    private fun startDimensionInput() {
        ...
        // Диагонали сортируем по алфавиту (BJ раньше чем CJ, и т.д.)
        val pointMap = binding.blueprintView.points.associateBy { it.id }
        val sortedDiags = binding.blueprintView.diagonals.sortedWith(Comparator { a, b ->
            val aFrom = pointMap[a.fromId]?.label ?: ""; val aTo = pointMap[a.toId]?.label ?: ""
            val bFrom = pointMap[b.fromId]?.label ?: ""; val bTo = pointMap[b.toId]?.label ?: ""
            val aKey = if (aFrom <= aTo) "$aFrom$aTo" else "$aTo$aFrom"
            val bKey = if (bFrom <= bTo) "$bFrom$bTo" else "$bTo$bFrom"
            aKey.compareTo(bKey)
        })
        allConstraints.addAll(sortedDiags)

        // Сохраняем ОРИГИНАЛЬНЫЕ координаты скетча для стабильной живой перестройки
        val pts = binding.blueprintView.points
        sketchOriginX = FloatArray(pts.size) { pts[it].x }
        sketchOriginY = FloatArray(pts.size) { pts[it].y }
    ...
    private fun showDiagonalPrompt(constraint: DrawLine, fromLabel: String, toLabel: String) {
        ...
        // Показываем буквы в алфавитном порядке (меньшая первая)
        val labelA = if (fromLabel <= toLabel) fromLabel else toLabel
        val labelB = if (fromLabel <= toLabel) toLabel else fromLabel
        binding.redBanner.setBackgroundColor(android.graphics.Color.parseColor("#FF8C00"))
        binding.redBanner.text = "ДИАГОНАЛЬ $labelA–$labelB  ($current/$totalDiags, ✓ пропустить)"
        binding.tvDimLabel.text = "Диаг. $labelA–$labelB, см"
        binding.tvDimDisplay.text = if (inputBuffer.isEmpty()) "0" else inputBuffer
        binding.blueprintView.activeLineId = constraint.id
        binding.blueprintView.activeDimIndex = currentDimIndex
        binding.blueprintView.invalidate()
    ...
    private fun confirmDimension() {
        ...
        if (isDimEditModeActive) {
            ...
            setNumpadVisible(false)
            // Сбрасываем режим редактирования (кнопка карандаш → серая)
            ...
            if (line.isDiagonal) {
                // Если это новая диагональ (от выбора углов) — добавляем в список
                val alreadyAdded = binding.blueprintView.diagonals.any { it.id == line.id }
                if (!alreadyAdded) {
                    binding.blueprintView.diagonals.add(line)
                }
                // Перестраиваем чертёж с учётом всех диагоналей
                rebuildWithDiagonals()
                Snackbar.make(binding.root, "Диагональ сохранена, чертёж перестроен ✓", Snackbar.LENGTH_SHORT).show()
            } else {
        ...
        // Живая перестройка: двигаем только замеренную цепочку от исходного скетча
        if (!constraint.isDiagonal && room.autoCorrection) {
            applyLiveWallCorrection()
        }
    ...
    /**
     * Живая перестройка чертежа во время ввода размеров стен.
     *
     * Алгоритм:
     * 1. Берём ИСХОДНЫЕ координаты скетча (сохранённые в sketchOriginX/Y) для определения
     *    направлений — они не меняются между итерациями, поэтому нет накопления ошибок.
     * 2. Вычисляем единый масштаб: суммарная длина в пикселях / суммарная длина в сантиметрах
     *    по всем уже замеренным стенам.
     * 3. Перестраиваем замеренную цепочку точек A→B→C→...
     *    от исходного положения точки A, используя измеренные длины × масштаб.
     * 4. Оставшиеся незамеренные точки сдвигаем на тот же вектор, что сдвинулась
     *    последняя замеренная точка, — фигура остаётся целой и всегда видна.
     */
    private fun applyLiveWallCorrection() {
        val origX = sketchOriginX ?: return
        val origY = sketchOriginY ?: return
        val pts = binding.blueprintView.points
        val wallLines = binding.blueprintView.lines.filter { !it.isDiagonal }
        val n = pts.size
        if (n < 2) return

        // Считаем только последовательно замеренные стены (без пропусков)
        var measuredCount = 0
        for (line in wallLines) {
            if (line.measuredLength != null && line.measuredLength!! > 0) measuredCount++
            else break
        }
        if (measuredCount == 0) return

        // Масштаб: суммарная длина замеренных стен в пикселях / в сантиметрах
        var totalMeasuredCm = 0.0
        var totalSketchPx = 0.0
        for (i in 0 until measuredCount.coerceAtMost(wallLines.size)) {
            val ml = wallLines.getOrNull(i)?.measuredLength ?: break
            totalMeasuredCm += ml
            if (i + 1 < origX.size) {
                val dx = (origX[i + 1] - origX[i]).toDouble()
                val dy = (origY[i + 1] - origY[i]).toDouble()
                totalSketchPx += kotlin.math.sqrt(dx * dx + dy * dy)
            }
        }
        if (totalMeasuredCm < 1e-9 || totalSketchPx < 1e-9) return
        val pxPerCm = totalSketchPx / totalMeasuredCm

        // Строим замеренную цепочку от исходного положения точки A
        pts[0].x = origX[0]
        pts[0].y = origY[0]

        var prevX = origX[0].toDouble()
        var prevY = origY[0].toDouble()

        for (i in 1..measuredCount.coerceAtMost(n - 1)) {
            val ml = wallLines.getOrNull(i - 1)?.measuredLength ?: break
            // Направление — из ИСХОДНОГО скетча, не из сдвинутых координат
            val dx = (origX[i] - origX[i - 1]).toDouble()
            val dy = (origY[i] - origY[i - 1]).toDouble()
            val segLen = kotlin.math.sqrt(dx * dx + dy * dy)
            if (segLen < 1e-9) {
                prevX = origX[i].toDouble()
                prevY = origY[i].toDouble()
                continue
            }
            val nx = dx / segLen; val ny = dy / segLen
            val newX = prevX + nx * ml * pxPerCm
            val newY = prevY + ny * ml * pxPerCm
            pts[i].x = newX.toFloat()
            pts[i].y = newY.toFloat()
            prevX = newX
            prevY = newY
        }

        // Незамеренные точки сдвигаем на тот же вектор что сдвинулась последняя замеренная
        val lastIdx = measuredCount.coerceAtMost(n - 1)
        if (lastIdx < origX.size && lastIdx < n - 1) {
            val shiftX = pts[lastIdx].x - origX[lastIdx]
            val shiftY = pts[lastIdx].y - origY[lastIdx]
            for (i in lastIdx + 1 until n) {
                if (i < origX.size) {
                    pts[i].x = origX[i] + shiftX
                    pts[i].y = origY[i] + shiftY
                }
            }
        }
    }
    ...
    private fun updateStats() {
        ...
        val diagText = diags
            .filter { d -> (d.measuredLength ?: 0.0) > 0 }
            .map { d ->
                val fromLabel = pts.find { it.id == d.fromId }?.label ?: "?"
                val toLabel = pts.find { it.id == d.toId }?.label ?: "?"
                val labelA = if (fromLabel <= toLabel) fromLabel else toLabel
                val labelB = if (fromLabel <= toLabel) toLabel else fromLabel
                val len = d.measuredLength!!.toInt()
                Triple("$labelA$labelB", len, "$labelA$labelB=$len")
            }
            .sortedBy { it.first }
        ...
        binding.tvDiagonals.visibility = if (diagText.isNotEmpty()) View.VISIBLE else View.GONE
    ...
    private fun showDimEditSheet() {
        if (!binding.blueprintView.isClosed) {
            Toast.makeText(this, "Сначала замкните фигуру", Toast.LENGTH_SHORT).show()
            return
        }
        val options = buildList {
            if (isAllDone) add("✏️ Изменить размер стены или диагонали")
            add("📐 Выбрать диагональ из списка")
            add("👆 Выбрать углы для диагонали на чертеже")
        }
        androidx.appcompat.app.AlertDialog.Builder(this)
            .setTitle("Диагонали и размеры")
            .setItems(options.toTypedArray()) { _, which ->
                when (options[which]) {
                    "✏️ Изменить размер стены или диагонали" -> showDimSizeListDialog()
                    "📐 Выбрать диагональ из списка" -> showAddDiagonalDialog()
                    "👆 Выбрать углы для диагонали на чертеже" -> enterDiagonalPickMode()
                }
            }
            .setNegativeButton(getString(R.string.cancel), null)
            .show()
    ...
    // ──────────────────────────────────────────────────────────────────────────
    // Список стен и диагоналей для редактирования/удаления
    // ──────────────────────────────────────────────────────────────────────────

    private fun showDimSizeListDialog() {
        val pts = binding.blueprintView.points
        val pointMap = pts.associateBy { it.id }
        val walls = binding.blueprintView.lines.filter { !it.isDiagonal }
        val sortedDiags = binding.blueprintView.diagonals.sortedWith(Comparator { a, b ->
            val aF = pointMap[a.fromId]?.label ?: ""; val aT = pointMap[a.toId]?.label ?: ""
            val bF = pointMap[b.fromId]?.label ?: ""; val bT = pointMap[b.toId]?.label ?: ""
            val aKey = if (aF <= aT) "$aF$aT" else "$aT$aF"
            val bKey = if (bF <= bT) "$bF$bT" else "$bT$bF"
            aKey.compareTo(bKey)
        })

        val lineItems = mutableListOf<DrawLine>()
        val displayItems = mutableListOf<String>()

        walls.forEach { line ->
            val from = pointMap[line.fromId]?.label ?: "?"
            val to   = pointMap[line.toId]?.label   ?: "?"
            val len  = line.measuredLength?.toInt()?.let { "$it см" } ?: "не задана"
            displayItems.add("Стена $from–$to = $len")
            lineItems.add(line)
        }
        sortedDiags.forEach { diag ->
            val from = pointMap[diag.fromId]?.label ?: "?"
            val to   = pointMap[diag.toId]?.label   ?: "?"
            val lA = if (from <= to) from else to
            val lB = if (from <= to) to else from
            val len = diag.measuredLength?.toInt()?.let { "$it см" } ?: "не задана"
            displayItems.add("Диагональ $lA–$lB = $len  🗑")
            lineItems.add(diag)
        }

        if (displayItems.isEmpty()) {
            Toast.makeText(this, "Нет стен или диагоналей", Toast.LENGTH_SHORT).show()
            return
        }

        androidx.appcompat.app.AlertDialog.Builder(this)
            .setTitle("Стены и диагонали")
            .setItems(displayItems.toTypedArray()) { _, which ->
                val line = lineItems[which]
                if (line.isDiagonal) {
                    showDiagonalEditOrDeleteDialog(line, pointMap)
                } else {
                    openLineEditingFromList(line, pointMap)
                }
            }
            .setNegativeButton(getString(R.string.cancel), null)
            .show()
    ...
    private fun showDiagonalEditOrDeleteDialog(diag: DrawLine, pointMap: Map<String, com.zamerpro.app.data.DrawPoint>) {
        val from = pointMap[diag.fromId]?.label ?: "?"
        val to   = pointMap[diag.toId]?.label   ?: "?"
        val lA = if (from <= to) from else to
        val lB = if (from <= to) to else from
        androidx.appcompat.app.AlertDialog.Builder(this)
            .setTitle("Диагональ $lA–$lB")
            .setItems(arrayOf("✏️ Изменить размер", "🗑️ Удалить диагональ")) { _, which ->
                when (which) {
                    0 -> openLineEditingFromList(diag, pointMap)
                    1 -> confirmDeleteDiagonal(diag, lA, lB)
                }
            }
            .setNegativeButton(getString(R.string.cancel), null)
            .show()
    ...
    private fun confirmDeleteDiagonal(diag: DrawLine, lA: String, lB: String) {
        androidx.appcompat.app.AlertDialog.Builder(this)
            .setTitle("Удалить диагональ $lA–$lB?")
            .setMessage("Диагональ будет удалена и чертёж перестроен")
            .setPositiveButton("Удалить") { _, _ ->
                binding.blueprintView.diagonals.remove(diag)
                room.diagonals.remove(diag)
                storage.saveRoom(project.id, room)
                rebuildWithDiagonals()
                updateStats()
                Snackbar.make(binding.root, "Диагональ $lA–$lB удалена", Snackbar.LENGTH_SHORT).show()
            }
            .setNegativeButton(getString(R.string.cancel), null)
            .show()
    }

    private fun openLineEditingFromList(line: DrawLine, pointMap: Map<String, com.zamerpro.app.data.DrawPoint>) {
        val from = pointMap[line.fromId]?.label ?: "?"
        val to   = pointMap[line.toId]?.label   ?: "?"
        val isDiag = line.isDiagonal
        val lA = if (isDiag && from > to) to else from
        val lB = if (isDiag && from > to) from else to
        val kind = if (isDiag) "Диаг." else "Стена"
        editingLine = line
        isDimEditModeActive = true
        val current = line.measuredLength?.toInt()?.toString() ?: ""
        inputBuffer = current
        binding.tvDimDisplay.text = if (inputBuffer.isEmpty()) "0" else inputBuffer
        binding.tvDimLabel.text = "$kind $lA–$lB, см"
        binding.blueprintView.activeLineId = line.id
        binding.blueprintView.invalidate()
        setNumpadVisible(true)
    }

    // ──────────────────────────────────────────────────────────────────────────
    // Режим выбора углов для диагонали на чертеже (тап по двум вершинам)
    // ──────────────────────────────────────────────────────────────────────────

    private fun enterDiagonalPickMode() {
        binding.blueprintView.isDiagonalPickMode = true
        binding.elementModeBar.visibility = View.VISIBLE
        binding.tvElementModeHint.text = "Нажмите на первый угол диагонали"
        binding.btnCancelElement.setOnClickListener { exitDiagonalPickMode() }
    }

    private fun exitDiagonalPickMode() {
        binding.blueprintView.isDiagonalPickMode = false
        binding.elementModeBar.visibility = View.GONE
        // Возвращаем кнопку Отмена к стандартному поведению (элементы потолка)
        binding.btnCancelElement.setOnClickListener { exitElementMode() }
    }

    /**
     * Показывает числовой ввод длины диагонали через встроенный нумпад
     * (вместо системного EditText-диалога) — вызывается при выборе углов на чертеже.
     */
    private fun showDiagonalLengthInputWithNumpad(p1: DrawPoint, p2: DrawPoint) {
        // Если такая диагональ уже есть — редактируем её, иначе создаём новую (не добавляем в список сразу)
        val existing = binding.blueprintView.diagonals.find {
            (it.fromId == p1.id && it.toId == p2.id) ||
            (it.fromId == p2.id && it.toId == p1.id)
        }
        val diagLine = existing ?: DrawLine(
            fromId = p1.id,
            toId = p2.id,
            isDiagonal = true
        )
        // Диагональ добавляется в список ТОЛЬКО после подтверждения в confirmDimension()

        editingLine = diagLine
        isDimEditModeActive = true

        val current = diagLine.measuredLength?.toInt()?.toString() ?: ""
        inputBuffer = current
        binding.tvDimDisplay.text = if (inputBuffer.isEmpty()) "0" else inputBuffer
        binding.tvDimLabel.text = "Диаг. ${p1.label}–${p2.label}, см"
        binding.blueprintView.activeLineId = existing?.id
        binding.blueprintView.invalidate()
        setNumpadVisible(true)
    }

    /**
     * Перестраивает чертёж с учётом ВСЕХ измеренных диагоналей.
     * Вызывается после добавления/изменения диагонали когда чертёж завершён.
     */
    private fun rebuildWithDiagonals() {
        try {
            val vw = binding.blueprintView.width.toFloat().coerceAtLeast(600f)
            val vh = binding.blueprintView.height.toFloat().coerceAtLeast(600f)
            val measuredDiags = binding.blueprintView.diagonals
                .filter { it.measuredLength != null && it.measuredLength!! > 0 }
            engine.correct(
                binding.blueprintView.points,
                binding.blueprintView.lines,
        ...
        androidx.appcompat.app.AlertDialog.Builder(this)
            ...
            .setPositiveButton("Добавить") { _, _ ->
                ...
                if (len != null && len > 0) {
                    ...
                    // Если чертёж завершён — перестраиваем; иначе только сохраняем метку
                    if (isAllDone) {
                        rebuildWithDiagonals()
                        Snackbar.make(binding.root,
                            "Диагональ ${p1.label}–${p2.label} добавлена, чертёж перестроен ✓",
